        ...
        */
        ...
        public function wholesale_role_filter_add_custom_fields( $taxonomy ) {
            ...
            <div class="form-field wholesale_role_filter">
                <label for="wholesale_role_filter"><?php _e( 'Wholesale Role Filter' , 'woocommerce-wholesale-prices-premium' ); ?></label>
                <select name="wholesale_role_filter[]" id="wholesale_role_filter" data-placeholder="<?php _e( 'Select Wholesale Roles...' , 'woocommerce-wholesale-prices-premium' ); ?>" multiple autocomplete="off" style="width: 95%;">
                    <?php foreach ( $all_registered_wholesale_roles  as $role_key => $role ) { ?>
                        <option value="<?php echo $role_key; ?>"><?php echo $role[ 'roleName' ]; ?></option>
                    <?php } ?>
                </select>
                <p class="description"><?php _e( 'Select wholesale roles this product category term is exclusive to.' , 'woocommerce-wholesale-prices-premium' ); ?></p>
            </div>
        ...
        public function wholesale_role_filter_edit_custom_fields( $term ) {
            ...
            <tr class="form-field wholesale_role_filter">
                <th scope="row" valign="top"><label for="wholesale_role_filter"><?php _e( 'Wholesale Role Filter' , 'woocommerce-wholesale-prices-premium' ); ?></label></th>
                <td>
                    <select name="wholesale_role_filter[]" id="wholesale_role_filter" data-placeholder="<?php _e( 'Select Wholesale Roles...' , 'woocommerce-wholesale-prices-premium' ); ?>" multiple autocomplete="off" style="width: 95%;">
                        <?php foreach ( $all_registered_wholesale_roles  as $role_key => $role ) { ?>
                            <option value="<?php echo $role_key; ?>" <?php echo in_array( $role_key , $specific_filter ) ? "selected" : ""; ?>><?php echo $role[ 'roleName' ]; ?></option>
                        <?php } ?>
                    </select>
                    <p class="description"><?php _e( 'Select wholesale roles this product category term is exclusive to.' , 'woocommerce-wholesale-prices-premium' ); ?></p>
                </td>
        ...
        */
                ...
                <td>
                    ...
                    <div id="per-order-quantity-wholesale-percent-discount-cat-level-controls-body">
                        ...
                        <label id="qty-mapping-mode-2-wrapper" for="enable-per-order-quantity-wholesale-percent-discount-cat-level-mode-2" style="display: inline-block; margin-bottom: 20px;">
                            <span class="spinner"></span>
                            <input type="checkbox" id="enable-per-order-quantity-wholesale-percent-discount-cat-level-mode-2" <?php echo ( $enable_mode2 == "yes" ) ? 'checked="checked"' : ''; ?> autocomplete="off">
                            <?php _e( 'By default, the category quantity based discounts system will use the total quantity of all items that belong to this category. This option changes this to apply category quantity based discounts based on the quantity of individual products in the cart' , 'woocommerce-wholesale-prices-premium' ); ?>
                        </label>
        ...
        public function toggle_cat_level_quantity_based_wholesale_discount_mode2() {
            ...
            if ( !defined( "DOING_AJAX" ) || !DOING_AJAX )
                $response = array( 'status' => '' , 'error_msg' => __( 'Invalid AJAX Call' , 'woocommerce-wholesale-prices-premium' ) );
            elseif ( !isset( $_POST[ 'term_id' ] ) || !isset( $_POST[ 'enable' ] ) )
                $response = array( 'status' => '' , 'error_msg' => __( 'Required parameters not supplied' , 'woocommerce-wholesale-prices-premium' ) );
            else {

                update_term_meta( $_POST[ 'term_id' ] , 'wwpp_enable_quantity_based_wholesale_discount_mode2' , $_POST[ 'enable' ] );
