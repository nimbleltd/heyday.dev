jQuery( document ).ready( function( $ ) {
    ...
    function generate_mapping_data( $field_controls ) {
        ...
        var wholesale_role_key  = $.trim( $field_controls.find( "#wwpp-wholesale-roles" ).val() ),
            wholesale_role_name = $.trim( $field_controls.find( "#wwpp-wholesale-roles" ).find( "option[value='" + wholesale_role_key + "']" ).text() ),
            tax_class           = $.trim( $field_controls.find( "#wwpp-tax-classes" ).val() ),
            tax_class_name      = $.trim( $field_controls.find( "#wwpp-tax-classes" ).find( "option[value='" + tax_class + "']" ).text() ),
            error               = [],
            data                = {};

        if ( wholesale_role_key === '' )
            error.push( wwpp_settings_tax_class_var.please_specify_wholesale_role );

        if ( tax_class === '' )
            error.push( wwpp_settings_tax_class_var.please_specify_tax_classes );
    ...
    function save_mapping_entry_to_table( $mapping_table , $mapping_entry_markup , mode , data ) {
        ...
        if ( $mapping_table.find( "tbody tr.no-items" ).length )
            $mapping_table.find( "tbody tr.no-items" ).remove();
        ...
        if ( mode === 'add' ) {
            ...
            $mapping_table
                .find( "tbody tr td.meta span.wholesale-role:contains('" + data[ 'wholesale-role-key' ] + "')" ).closest( 'tr' )
    ...
    function remove_mapping_entry_to_table( $mapping_table , wholesale_role_key ) {
        ...
        $mapping_table
            .find( "tbody tr td.meta span.wholesale-role:contains('" + wholesale_role_key + "')" ).closest( 'tr' )
            .remove();

        if ( $mapping_table.find( "tbody tr" ).length <= 0 ) {

            $mapping_table.find( "tbody" ).append( '<tr class="no-items">' +
                                                        '<td class="colspanchange" colspan="3">' + wwpp_settings_tax_class_var.no_mappings_found + '</td>' +
                                                   '</tr>' );
    ...
    function prepopulate_field_controls( $field_controls , wholesale_role_key , tax_class ) {
        ...
        $field_controls.find( "#wwpp-wholesale-roles" ).val( wholesale_role_key ).attr( 'disabled' , 'disabled' ).trigger( 'chosen:updated' );
    ...
    function save_mapping_entry( mode ) {
        ...
        if ( d.error.length > 0 ) {
            ...
            toastr.error( err_msg , wwpp_settings_tax_class_var.form_error , { "closeButton" : true , "showDuration" : error_message_duration } );
            ...
            $.ajax( {
                url      : ajaxurl,
                type     : "POST",
                data     : { action : "wwpp_save_tax_class_mapping" , "mapping-data" : d.data },
                dataType : "json"
            } )
            .done( function( data ) {
                ...
                if ( data.status === 'success' ) {
                    ...
                    console.log( data );
                    toastr.error( data.error_msg , wwpp_settings_tax_class_var.form_error , { "closeButton" : true , "showDuration" : error_message_duration } );
            ...
            .fail( function( jqxhr ) {
                ...
                console.log( jqxhr );
                toastr.error( wwpp_settings_tax_class_var.failed_save_mapping_entry , wwpp_settings_tax_class_var.form_error , { "closeButton" : true , "showDuration" : error_message_duration } );
    ...
    $add_btn.click( function() { save_mapping_entry( 'add' ); } );
    ...
    $save_btn.click( function() { save_mapping_entry( 'edit' ); } );
    ...
    $mapping_table.on( "click" , "tbody tr td.controls .delete" , function() {
        ...
        if ( confirm( wwpp_settings_tax_class_var.confirm_delete_mapping_entry ) ) {
            ...
            $.ajax( {
                url      : ajaxurl,
                type     : "POST",
                data     : { action : 'wwpp_delete_tax_class_mapping' , 'wholesale-role-key' : wholesale_role_key },
                datatype : "json"
            } )
            .done( function( data ) {
            ...
            .fail( function( jqxhr ) {
                ...
                console.log( jqxhr );
                toastr.error( wwpp_settings_tax_class_var.failed_delete_mapping_entry , wwpp_settings_tax_class_var.form_error , { "closeButton" : true , "showDuration" : error_message_duration } );
