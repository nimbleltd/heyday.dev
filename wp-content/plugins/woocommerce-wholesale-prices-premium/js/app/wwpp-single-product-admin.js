...
jQuery( document ).ready( function( $ ) {
    ...
    function validate_empty_fields( wholesale_role , min , max , price_type , price , currency ) {

        var err_fields = [];

        if ( wholesale_role == '' )
            err_fields.push( wwpp_single_product_admin_params.i18n_wholesale_role )
        ...
        if ( price == '' )
            err_fields.push( wwpp_single_product_admin_params.i18n_wholesale_price );
    ...
    function reset_fields( $parent_fields_container ) {
        ...
        $parent_fields_container.find( ".pqbwp_price_type" ).find( "option:first-child" ).attr( 'selected' , 'selected' );
        $parent_fields_container.find( ".pqbwp_wholesale_price" ).val( '' );

        if ( $parent_fields_container.find( ".pqbwp_enabled_currencies" ).length > 0 )
            $parent_fields_container.find( ".pqbwp_enabled_currencies" ).find( "option:contains('" + wwpp_single_product_admin_params.i18n_base_currency + "')" ).attr( 'selected' , 'selected' );
    ...
    $( 'body' ).delegate( '.pqbwp_price_type' , "change" , function() {
        ...
        var $this        = $( this ),
            $price_field = $this.closest( 'p' ).siblings( '.pqbwp_wholesale_price_field' );
            
        if ( $this.val() == 'fixed-price' )
            $price_field.find( 'label' ).text( wwpp_single_product_admin_params.i18n_fixed_price_wholesale_label );
        else if ( $this.val() == 'percent-price' )
            $price_field.find( 'label' ).text( wwpp_single_product_admin_params.i18n_percent_price_wholesale_label );
        ...
        wwppBackendAjaxServices.toggle_product_quantity_based_wholesale_pricing( post_id , enable )
            .done( function( data , textStatus , jqXHR ) {
                ...
                if ( data.status == 'success' ) {
                    ...
                    if ( enable == 'yes' )
                        err_msg = wwpp_single_product_admin_params.i18n_fail_enable_product_quantity;
                    else
                        err_msg = wwpp_single_product_admin_params.i18n_fail_disable_product_quantity;
    ...
    $( 'body' ).delegate( '.pqbwp-add-rule' , 'click' , function() {
        ...
        var wholesale_role      = $.trim( $parent_fields_container.find( ".pqbwp_registered_wholesale_roles" ).val() ),
            wholesale_role_text = $.trim( $parent_fields_container.find( ".pqbwp_registered_wholesale_roles" ).find( "option:selected" ).text() ),
            ...
            price_type          = $.trim( $parent_fields_container.find( ".pqbwp_price_type" ).val() ),
        ...
        if ( currency_field.length > 0 ) {
            ...
            err_fields = validate_empty_fields( wholesale_role , start_qty , end_qty , price_type , wholesale_price , currency );

        } else
            err_fields = validate_empty_fields( wholesale_role , start_qty , end_qty , price_type , wholesale_price );

        if ( err_fields.length > 0 ) {

            var err_msg = wwpp_single_product_admin_params.i18n_fill_fields_properly + '<br/><br/></ul>';
            ...
            toastr.error( err_msg , wwpp_single_product_admin_params.i18n_fill_form_properly , { "closeButton" : true , "showDuration" : error_message_duration } );
        ...
        if ( !validate_quantity_fields( start_qty , end_qty ) ) {
            ...
            toastr.error( '' , wwpp_single_product_admin_params.i18n_ending_qty_must_not_be_less_start_qty , { "closeButton" : true , "showDuration" : error_message_duration } );
        ...
        var $role_wholesale_price_field = $( "#variable_product_options input[ id ^= '" + wholesale_role + "_wholesale_prices' ]" ).length ? $( "#variable_product_options input[ id ^= '" + wholesale_role + "_wholesale_prices' ]" ) : $( "#general_product_data .pricing #" + wholesale_role + "_wholesale_price" );
        if ( $role_wholesale_price_field.val() === '' )
            alert( wwpp_single_product_admin_params.required_base_wholesale_price_err_msg[ wholesale_role ] );
        ...
        wwppBackendAjaxServices.addQuantityDiscountRule( post_id , rule )
            .done( function( data , textStatus , jqXHR ) {
                ...
                if ( data.status == 'success' ) {
                    ...
                    toastr.success( '' , wwpp_single_product_admin_params.i18n_success_add_discount_mapping , { "closeButton" : true , "showDuration" : success_message_duration } );
                    ...
                    $table_mapping.find( "tbody" )
                        .append( '<tr class="' + tr_class + ' edited">' +
                                    '<td class="meta hidden">' +
                                        ...
                                        '<span class="price-type">' + price_type + '</span>' +
                    ...
                    toastr.error( data.error_message , wwpp_single_product_admin_params.i18n_fail_add_discount_mapping , { "closeButton" : true , "showDuration" : error_message_duration } );

                    console.log( wwpp_single_product_admin_params.i18n_fail_add_discount_mapping );
                    console.log( data );
                    console.log( '----------' );
            ...
            .fail( function( jqXHR , textStatus , errorThrown ) {
                ...
                toastr.error( jqXHR.responseText , wwpp_single_product_admin_params.i18n_fail_add_discount_mapping , { "closeButton" : true , "showDuration" : error_message_duration } );
    ...
    $( 'body' ).delegate( '.pqbwp-save-rule' , 'click' , function() {
        ...
        wwppBackendAjaxServices.saveQuantityDiscountRule( post_id , index , rule )
                ...
                if ( data.status == 'success' ) {
                    ...
                    $table_mapping.find( "tr.edited" )
                        .find( ".meta" )
                            ...
                            .find( ".price-type" ).text( price_type ).end()
                            .end()
                    ...
                    toastr.success( '' , wwpp_single_product_admin_params.i18n_success_update_discount_mapping , { "closeButton" : true , "showDuration" : success_message_duration } );
                    ...
                    toastr.error( data.error_message , wwpp_single_product_admin_params.i18n_fail_update_discount_mapping , { "closeButton" : true , "showDuration" : error_message_duration } );

                    console.log( wwpp_single_product_admin_params.i18n_fail_update_discount_mapping );
                    console.log( data );
                    console.log( '----------' );
            ...
            .fail( function( jqXHR , textStatus , errorThrown ) {
                ...
                toastr.error( jqXHR.responseText , wwpp_single_product_admin_params.i18n_fail_update_discount_mapping , { "closeButton" : true , "showDuration" : error_message_duration } );
    ...
    $( 'body' ).delegate( '.pqbwp-mapping .edit' , 'click' , function() {
        ...
        var $this                    = $( this ),
            ...
            price_type               = $.trim( $current_tr.find( ".meta" ).find( ".price-type" ).text() ),
    ...
    $( 'body' ).delegate( '.pqbwp-mapping .delete' , 'click' , function() {
        ...
        if ( confirm( wwpp_single_product_admin_params.i18n_click_ok_remove_discount_mapping ) ) {
            ...
            wwppBackendAjaxServices.deleteQuantityDiscountRule( post_id , index )
                .done( function ( data , textStatus , jqXHR ) {
                    ...
                    if ( data.status == 'success' ) {
                            ...
                            if ( $table_mapping.find( "tbody" ).find( "tr" ).length <= 0 ) {
                                ...
                                $table_mapping
                                    ...
                                    .html(  '<tr class="no-items">' +
                                                '<td class="colspanchange" colspan="5">' + wwpp_single_product_admin_params.i18n_no_quantity_discount + '</td>' +
                                            '</tr>' );
                        ...
                        toastr.success( '' , wwpp_single_product_admin_params.i18n_success_delete_discount , { "closeButton" : true , "showDuration" : success_message_duration } );

                    } else {

                        toastr.error( data.error_message , wwpp_single_product_admin_params.i18n_fail_delete_discount , { "closeButton" : true , "showDuration" : error_message_duration } );

                        console.log( wwpp_single_product_admin_params.i18n_fail_delete_discount );
                        console.log( data );
                        console.log( '----------' );
                ...
                .fail( function ( jqXHR , textStatus , errorThrown ) {
                    ...
                    toastr.error( jqXHR.responseText , wwpp_single_product_admin_params.i18n_fail_delete_discount , { "closeButton" : true , "showDuration" : error_message_duration } );
