jQuery( document ).ready( function( $ ) {
    ...
    function filter_number_input( e ) {
        ...
        if ( element_id === 'wwpp_min_order_price' || element_id === 'wwpp_wholesale_discount' )
            filter_arr.push( 190 ); // Allow .

            // Allow: backspace, delete, tab, escape, enter and .
        if ( $.inArray( e.keyCode , filter_arr ) !== -1 ||
    ...
    $override_min_order_qty.change( function() {
        ...
        if ( $( this ).val() === 'yes' ) {
            ...
            if ( $override_min_order_price.val() === 'yes' )
                $form.find( "#wwpp_min_order_logic-tr" ).slideDown( 'fast' );
            else
    ...
    $override_min_order_price.change( function() {
        ...
        if ( $( this ).val() === 'yes' ) {
            ...
            if ( $override_min_order_qty.val() === 'yes' )
                $form.find( "#wwpp_min_order_logic-tr" ).slideDown( 'fast' );
            else
                $form.find( "#wwpp_min_order_logic-tr" ).slideUp( 'fast' );
            ...
            if ( data.status == 'success' ) {
                ...
                for ( var method_instance_id in data.shipping_methods  ) {
                    ...
                    if ( data.shipping_methods.hasOwnProperty( method_instance_id ) ) {
                        ...
                        var selected = '';
                        if ( zone_id == wwpp_user_profile_args.wwpp_shipping_zone && wwpp_user_profile_args.wwpp_zone_shipping_methods )
                            selected = wwpp_user_profile_args.wwpp_zone_shipping_methods.indexOf( parseInt( method_instance_id , 10 ) ) > -1 ? 'selected' : '';
    ...
    $specify_non_zoned_method.change( function() {
        ...
        if ( $( this ).val() === 'yes' )
            $form.find( "#wwpp_non_zoned_shipping_methods-tr" ).slideDown( "fast" );
        else
    ...
    $override_payment_gateway_options.change( function() {
        ...
        if ( $( this ).val() === "yes" )
            $form.find( "#wwpp_payment_gateway_options-tr" ).slideDown( "fast" );
        else
            $form.find( "#wwpp_payment_gateway_options-tr" ).slideUp( "fast" );
    ...
    $override_payment_gateway_surcharge.change( function() {
        ...
        if ( $( this ).val() === "specify_surcharge_mapping" )
            ...
            $form.find( "#wwpp_payment_gateway_surcharge_mapping-tr" ).slideUp( "fast" );
    ...
    $( "#submit" ).click( function( e ) {
        ...
        if ( shipping_methods && shipping_methods.length > 0 ) {
            ...
            for ( var i = 0 ; i < shipping_methods.length ; i++ )
                $form.find( "#wwpp_shipping_methods-tr" ).append( '<input type="hidden" name="wwpp_shipping_methods[]" value="' + shipping_methods[i] + '">' );

        } else
            $form.find( "#wwpp_shipping_methods-tr" ).append( '<input type="hidden" name="wwpp_shipping_methods" value="">' );
        ...
        if ( non_shipping_methods && non_shipping_methods.length > 0 ) {
            ...
            for ( var i = 0 ; i < non_shipping_methods.length ; i++ )
                $form.find( "#wwpp_non_zoned_shipping_methods-tr" ).append( '<input type="hidden" name="wwpp_non_zoned_shipping_methods[]" value="' + non_shipping_methods[i] + '">' );

        } else
            $form.find( "#wwpp_non_zoned_shipping_methods-tr" ).append( '<input type="hidden" name="wwpp_non_zoned_shipping_methods" value="">' );
        ...
        if ( payment_gateways && payment_gateways.length > 0 ) {
            ...
            for ( var i = 0 ; i < payment_gateways.length ; i++ )
                $form.find( "#wwpp_payment_gateway_options-tr" ).append( '<input type="hidden" name="wwpp_payment_gateway_options[]" value="' + payment_gateways[i] + '">' );

        } else
            $form.find( "#wwpp_payment_gateway_options-tr" ).append( '<input type="hidden" name="wwpp_payment_gateway_options" value="">' );
